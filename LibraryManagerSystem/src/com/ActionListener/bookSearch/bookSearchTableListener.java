...
public class bookSearchTableListener extends MouseAdapter {
	@Override
   	...
   	public static JPopupMenu makePopup(){
       	...
       	delete.addActionListener(new ActionListener() {
   			...
   			public void actionPerformed(ActionEvent e) {
   				...
   				JTable table=bookSearchFrame.table;
   				int[] rows = table.getSelectedRows();
   				...
   				for (int i = 0; i < rows.length; i++) {
   					...
   					readISBN[i]=table.getValueAt(rows[i], 4).toString().trim();
   					map.put(bookISBN[i], readISBN[i]);
   				}
   				Book_Borrow_Dao book_Borrow_Dao=new Book_Borrow_Dao();
   				try {						
   					if(book_Borrow_Dao.deleteBook_Borrow(map)){
   						DefaultTableModel tablemodel = new DefaultTableModel(bookSearchFrame.getSearchBookInfoAll(new Book_SearchDao().selectSearchBookInfoAll()),Constant.searchDetailcolumnNames) {
   			 				@Override
